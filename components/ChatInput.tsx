...
export function ChatInput({ onSend, onStop, isLoading, disabled, variant = 'footer' }: ChatInputProps) {
  ...
  return (
    <div className={containerClasses}>
      <form onSubmit={(e) => { e.preventDefault(); handleSubmit() }} className="relative flex w-full items-end gap-2">
        <textarea
        ...
        {isLoading ? (
          <button
            type="button"
            onClick={onStop}
            className="absolute right-1.5 bottom-1.5 cursor-pointer rounded-full bg-destructive p-2 text-destructive-foreground shadow-sm transition-all hover:bg-destructive/80"
            title="Stop"
          >
          ...
          <button
            ...
            className="absolute right-1.5 bottom-1.5 cursor-pointer rounded-full bg-primary p-2 text-primary-foreground shadow-sm transition-all hover:bg-primary/80 disabled:opacity-50"
          >
